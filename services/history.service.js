...
const historyService = {
    ...
    getTOP: async () => {
        ...
        let list = await historyModel.aggregate([{ $match: { status: 'win', updatedAt: { $gte: moment().startOf(dataSetting.topData.typeTOP).toDate(), $lt: moment().endOf(dataSetting.topData.typeTOP).toDate() } } }, { $group: { _id: "$partnerId", amount: { $sum: '$bonus' } } }, { $sort: { amount: -1 } }, { $limit: dataTOP.length }]);
    ...
    getHistory: async () => {
        let historys = await historyModel.find({ $or: [{ status: 'win' }, { status: 'won' }] }).sort({ createdAt: 'desc' }).limit(10);
        let list = [];

        for (const history of historys) {
            list.push({
                transId :history.transId,
                transNew: history.transNew,
		        random: history.random,
                amount: Number(history.amount),
                bonus: Number(history.bonus),
                gameName: history.gameName,
                content: history.comment,
                status: history.status,
                time: moment(history.timeTLS).format('HH:mm:ss'),
